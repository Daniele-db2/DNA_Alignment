                ...
                for pos_gen in PG[z]:
                    ...
                    if ((100 - (D[len(D) - 1][len(D) - 1] / float(len(dict[i]))) * 100) < 60.0):
                        ...
                        print("-", 100 - (D[len(D) - 1][len(D) - 1] / float(len(dict[i]))) * 100,
                              "% ---> Local alignment")
                        A, optloc = Aligner.local_align(dict[i], genome[
                                                                 pos_gen - seedArray[z]: pos_gen - seedArray[z] + len(
                                                                     dict[i])], Aligner.ScoreParam())  # Smith-Waterman
                    else:
                        print("-", 100 - (D[len(D) - 1][len(D) - 1] / float(len(dict[i]))) * 100,
                              "% ---> Global alignment")
                        M = Aligner.affine_align(dict[i],
                                                 genome[pos_gen - seedArray[z]: pos_gen - seedArray[z] + len(dict[i])],
                                                 Aligner.ScoreParam())  # Needleman-Wunsch
                    if optloc == None:
